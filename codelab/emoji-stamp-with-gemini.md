...
```
gradle/libs.versions.toml で管理しているライブラリを最新の安定版に更新してください。
更新後、ビルドが通ることを確認してください。
```

Agent は各ライブラリの最新バージョンを調べて計画を提示します。更新対象とバージョンを確認して進めましょう。ファイル書き込みの許可（**Always allow** を選ぶチャンスです）のあと、ビルドでの検証まで Agent が行い、完了すると **Changes** パネルに差分が表示されます。`libs.versions.toml` の差分を確認して **Keep All** したら、コミットしておきましょう。

> aside positive
> このプロジェクトは依存関係を**バージョンカタログ**（`gradle/libs.versions.toml`）で一元管理しているので、「このファイルを最新に」という短い指示で全ライブラリの更新が完結します。実務でも、プロジェクトの構成が整理されているほど Agent への指示は短くて済みます。
