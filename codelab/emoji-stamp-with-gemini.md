...
![Commit ツールウィンドウ。ファイルをすべてチェックし、メッセージを入力して Commit](img/06-initial-commit.png)
...
Agent パネルの **「+」（New Conversation）** で会話を追加すると、**複数のエージェントタスクを並列に実行**できます。**Recent Chats** で会話を切り替えて進捗を確認でき、会話ごとに異なるモデルを選ぶことも可能です（`File > New > New Agent Tab` で専用タブも開けます）。
