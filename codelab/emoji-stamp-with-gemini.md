...
Gemini in Android Studio には大きく2つの機能があります。

| 機能 | できること |
| --- | --- |
| **Chat** | 質問応答・コード説明・レビュー。ファイルは書き換えない |
| **Agent** | 指示に従って**複数ファイルを編集**し、ビルド・エラー修正まで自律的に進める。ファイル検索、コマンド実行、デバイス操作（スクリーンショット・Logcat 確認）などのツールを持つ |

Agent は、ツールウィンドウバーの **Agent** アイコンから専用パネルとして開きます。

### 並列エージェントチャット

Agent パネルの **「+」（New Chat）** で会話を追加すると、**複数のエージェントタスクを並列に実行**できます。**Recent Chats** で会話を切り替えて進捗を確認でき、会話ごとに異なるモデルを選ぶことも可能です（`File > New > New Agent Tab` で専用タブも開けます）。

![TODO: 複数のエージェントチャットを並列で開いた状態。「+」と Recent Chats がわかるように](img/09-parallel-chats.png)
...
`File > Settings > Tools > Gemini`（macOS は `Android Studio > Settings...`）に Gemini の設定がまとまっています。本編に入る前に2つ確認しておきましょう。

* **コンテキスト共有（プライバシー）**：コードをコンテキストとして送信するかどうかはここで制御します。コードは同意なしにマシンの外へ送られません。なお**無料枠では共有したデータが AI の改善に使われる場合があります**（有償プランでは使われません）。実務では組織のポリシーに合わせて設定・プラン選択をしてください
* **Agent の権限**：Agent がコマンド実行など踏み込んだ操作をするときは許可を求めてきます。**Allow**（今回だけ）と **Always allow**（常に許可）を使い分けます。ハンズオン中は都度確認で進めましょう

チャット欄のモデルピッカーで使用モデルの切り替えもできます（設定からリモートモデルを追加することも可能）。
...
![TODO: プロジェクト直下に AGENTS.md を作成してエディタで開いた状態](img/15-agents-md.png)

> aside positive
> IDE の設定画面にある **Rules** も同様の仕組みですが、あちらは `.idea` 配下に保存される**個人設定**です。**チームで共有する規約は AGENTS.md、個人の好みは Rules** と使い分けます。AGENTS.md はディレクトリごとに置けるので、モジュール別の指示も可能です。実務では「アーキテクチャ方針」「テスト方針」「命名規則」を AGENTS.md に落とすところから始めると、Agent の出力がチームのコードベースに馴染みます。
...
1. [github.com/android/skills](https://github.com/android/skills) から使いたいスキルをダウンロード
...
> aside negative
> スキルの配置ディレクトリ（`.agents/skills/`）は Android Studio Quail 1 以降の仕様です。それより前のバージョンでは場所が異なるため、最新版を使ってください。
...
* **コードレビュー依頼**：Agent とは別の Chat 機能で「`MainActivity.kt` をレビューして。再コンポジションの観点で問題はない？」
* **Android Skills を試す**：[android/skills](https://github.com/android/skills) からスキル（例：テスト系や adaptive 系）を `.agents/skills/` に配置し、`@skill-name` で呼び出してみる
