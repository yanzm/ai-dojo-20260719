...
<img src="img/01-finished-app.png" width="400" alt="完成アプリ。写真の上に絵文字スタンプが複数配置されている" />
...
<img src="img/32-update-libs-prompt.png" width="460" alt="Agent への依頼：ライブラリを最新の安定版に更新してください" />

Agent はバージョンカタログ（`gradle/libs.versions.toml`）を見つけ、**Web で各ライブラリの最新バージョンを調べて**更新計画を提示します。更新対象とバージョン、検証の進め方を確認して「はい」で進めましょう。

![Agent が提示したライブラリの更新計画。調査結果・変更内容・検証計画が含まれる](img/33-update-libs-plan.png)

ファイル書き込みの許可を求められたら、ここで **Always allow** を選びましょう。以後の書き込み確認がスキップされ、作業が止まらなくなります。

<img src="img/34-update-libs-always-allow.png" width="465" alt="書き込み許可のプロンプト。Always allow を選ぶ" />
...
![Always allow を選ぶと Agent Permissions の Write source files in the project だけが Always allow に変わる](img/36-write-permission-setting.png)

ビルドでの検証まで完了すると、Walkthrough に更新内容と検証結果がまとまり、**Changes** パネルに差分が表示されます。`libs.versions.toml` の差分を確認して **Keep All** します。

![完了後の画面。Walkthrough に更新内容と検証結果、Changes パネルの Keep All で確定](img/35-update-libs-keep-all.png)
...
<img src="img/40-agent-commit-prompt.png" width="475" alt="コミットと .gitignore チェックの依頼を入力した状態" />
...
<img src="img/37-agent-commit-gitignore.png" width="485" alt="Agent が git status を確認し、.gitignore への追加を提案している" />

「はい」で進めると `.gitignore` を更新し、コミット対象を再確認してくれます。

<img src="img/38-agent-commit-status.png" width="485" alt=".gitignore を更新し、コミット対象を確認している" />
...
<img src="img/39-agent-commit-allowlist.png" width="450" alt="git add と git commit の実行許可。それぞれ allow list に追加するチェックボックス付き" />
...
計画に問題がなければ「はい」と返信します。Agent は Task List を作って作業を進めます（ウォームアップで **Always allow** を選んでいれば、書き込みの確認なしで進みます。差分のレビューは完了後の Changes パネルでまとめて行いましょう）。
...
<img src="img/12-camera-capture.png" width="400" alt="「カメラで撮影」ボタンが追加され、撮影した写真が表示された画面" />
...
<img src="img/13-emoji-row.png" width="400" alt="画面下部に絵文字パレットが横並びし、選択中の絵文字が枠線で強調されている" />
...
<img src="img/14-emoji-stamped.png" width="400" alt="写真の上に星とハートのスタンプが複数配置された画面" />
