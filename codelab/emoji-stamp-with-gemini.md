...
<img src="img/01-01-finished-app.png" width="400" alt="完成アプリ。写真の上に絵文字スタンプが複数配置されている" />
...
![New Project のテンプレート選択画面。Empty Activity を選択](img/02-01-new-project-template.png)

3. **Name** に `EmojiStamp` と入力し、他はデフォルトのまま **Finish**

![プロジェクト設定画面。Name に EmojiStamp と入力](img/02-02-new-project-settings.png)
...
![Agent アイコンの場所と「Project context required」の確認。Proceed を押す](img/02-03-project-context-required.png)

3. 未サインインの場合は **「Welcome to Gemini」** が表示されるので、**Sign in to Google** からサインインします（事前準備でサインイン済みの場合は表示されません）

![Welcome to Gemini 画面。Sign in to Google からサインイン](img/02-04-welcome-to-gemini.png)

### 動作確認

1. Gradle Sync 完了後、▶ Run でエミュレータ（または実機）に「Hello Android」が表示されることを確認

![ツールバーの ▶ Run ボタンでアプリを実行する。エミュレータは右側の Running Devices ツールウィンドウに表示される](img/02-05-run-button-running-devices.png)
...
![Build Output のエラー。末尾のクイックフィックスリンクをクリック](img/02-06-compilesdk-error.png)

2. **Refactoring Preview** が開き、`app/build.gradle.kts` の `compileSdk` の更新内容が表示されるので、**Refactor** をクリックします

![Refactoring Preview で更新内容を確認して Refactor をクリック](img/02-07-compilesdk-refactor.png)
...
<img src="img/02-08-initial-commit.png" width="400" alt="Commit ツールウィンドウ。ファイルをすべてチェックし、メッセージを入力して Commit" />
...
<img src="img/03-01-parallel-chats.png" width="600" alt="Recent Chats に2つの会話が並んだ状態。「+」で会話を追加し、Recent Chats で切り替える" />
...
![Settings > Tools > AI。左のツリーに各サブページ、右に Enable Pre-Bundled Skills](img/03-02-ai-settings.png)

使用モデルは Agent パネルの入力欄右下のピッカーで切り替えます（会話ごとに選択できます）。

<img src="img/03-03-model-picker.png" width="580" alt="入力欄右下のモデルピッカー。会話ごとにモデルを選べる（この例は Google AI Studio のモデルを追加した状態。デフォルトでは Gemini の項目のみ表示されます）" />
...
![Model Providers に Google AI Studio の API キーを登録し、使いたいモデルを有効化](img/03-04-model-provider-ai-studio.png)

![サードパーティプロバイダの追加画面（例：OpenAI）。データ送信の警告が表示される](img/03-05-model-provider-openai.png)
...
<img src="img/03-06-create-agents-md.png" width="640" alt="表示モードを Project に切り替え、ルートを右クリックして New > File" />
...
![AGENTS.md の実験用ルールが効いて、Agent がギャル口調＋絵文字で返事している](img/03-07-agents-md-test.png)
...
![プロジェクトルートの AGENTS.md をプロジェクトの指示に置き換えた状態](img/03-08-agents-md-final.png)
...
<img src="img/03-09-skills-list.png" width="580" alt="入力欄で @ を入力すると、スキルと Saved Prompts、ファイルの候補が表示される" />

一覧からスキルを選ぶと、チップとして入力欄に入ります。

<img src="img/03-10-skill-selected.png" width="570" alt="選択したスキルはチップとして入力欄に入る" />

例えば `edge-to-edge` スキルを付けてエッジツーエッジ対応を依頼すると、Agent がまずスキルを読み込み（Activate Skill）、その手順に沿って作業を進めます。実行までは、後の自由演習で試してみてください。

<img src="img/03-11-skill-activated.png" width="590" alt="Agent が edge-to-edge スキルを読み込んで（Activate Skill / Read skill）作業を進める様子" />

### 差分レビューの観点

Agent の変更に対する主なレビューポイントは2つ——**作業前の実装計画**と、**完了後の Changes パネル**（Keep All / Revert All）です。すべての行を精読する必要はありません。まずは次のような観点で、差分をざっと眺めることから始めましょう：

* 頼んでいないファイルまで書き換えていないか
* 不要な依存関係が `build.gradle.kts` に追加されていないか
* `AndroidManifest.xml` に**不要な権限が追加されていないか**（後のステップで実例が出ます）
...
<img src="img/03-12-write-permission.png" width="530" alt="Agent のファイル書き込み許可プロンプト。Allow / Cancel and do not allow / Always allow" />
...
機能実装に入る前に、小さく完結したタスクで Agent との開発サイクルを一周してみましょう。テンプレートから生成したプロジェクトは依存ライブラリのバージョンが最新でないことがあるので、これを Agent に更新してもらいます。

```
ライブラリを最新の安定版に更新してください。
```

<img src="img/04-01-update-libs-prompt.png" width="460" alt="Agent への依頼：ライブラリを最新の安定版に更新してください" />

Agent はバージョンカタログ（`gradle/libs.versions.toml`）を見つけ、**Web で各ライブラリの最新バージョンを調べて**更新計画を提示します。更新対象とバージョン、検証の進め方を確認して「はい」で進めましょう。

![Agent が提示したライブラリの更新計画。調査結果・変更内容・検証計画が含まれる](img/04-02-update-libs-plan.png)

> aside positive
> ファイル名を指定しなくても、Agent はプロジェクトを調べて**バージョンカタログ**（`gradle/libs.versions.toml`）にたどり着きます。依存関係が一元管理されているプロジェクトなら、指示はこの一言で済みます。実務でも、構成が整理されているほど Agent への指示は短くなります。

ファイル書き込みの許可を求められたら、ここで **Always allow** を選びましょう。以後の書き込み確認がスキップされ、作業が止まらなくなります。

<img src="img/04-03-update-libs-always-allow.png" width="465" alt="書き込み許可のプロンプト。Always allow を選ぶ" />

この選択は `Settings > Tools > AI > Agent Permissions` に反映されます。変わるのは **Write source files in the project**（プロジェクト内のソースファイル書き込み）だけで、プロジェクト外へのアクセスや削除などは都度確認のままです。設定はここからいつでも戻せます。

![Always allow を選ぶと Agent Permissions の Write source files in the project だけが Always allow に変わる](img/04-04-write-permission-setting.png)

ビルドでの検証まで完了すると、Walkthrough に更新内容と検証結果がまとまり、**Changes** パネルに差分が表示されます。`libs.versions.toml` の差分を確認して **Keep All** します。

![完了後の画面。Walkthrough に更新内容と検証結果、Changes パネルの Keep All で確定](img/04-05-update-libs-keep-all.png)

最後に、コミットも Agent に頼んでみましょう。テンプレート直後のプロジェクトには追跡不要なファイルが混ざりがちなので、`.gitignore` のチェックも一緒に依頼するのがおすすめです。

```
ここまでの変更をコミットしたい。
.gitignore に追加したほうがいいものがあるかチェックしてください。
```

<img src="img/04-06-agent-commit-prompt.png" width="475" alt="コミットと .gitignore チェックの依頼を入力した状態" />

Agent は `git status` で状況を確認し、エージェントの作業ログ（`.artifacts/`）や IDE の設定ファイル・ビルドキャッシュを `.gitignore` に追加するよう提案してくれます。AGENTS.md については「リポジトリで管理する想定ならコミットに含める」という妥当な判断も添えてくれました。

<img src="img/04-07-agent-commit-gitignore.png" width="485" alt="Agent が git status を確認し、.gitignore への追加を提案している" />

「はい」で進めると `.gitignore` を更新し、コミット対象を再確認してくれます。

<img src="img/04-08-agent-commit-status.png" width="485" alt=".gitignore を更新し、コミット対象を確認している" />

シェルコマンドの実行は、ファイル書き込みとは別に都度許可が求められます。「Add `git add` to allow list」「Add `git commit` to allow list」にチェックを入れて **Allow** すれば、この2つのコマンドは以後確認なしで実行されます。

<img src="img/04-09-agent-commit-allowlist.png" width="450" alt="git add と git commit の実行許可。それぞれ allow list に追加するチェックボックス付き" />

allow list に追加したコマンドは、**Settings > Tools > AI > Agent Permissions** の **Always allowed shell commands** に登録されます。あとから一覧を確認したり、不要になったコマンドを削除したりできます。

![Agent Permissions の Always allowed shell commands に git add と git commit が登録されている](img/04-10-shell-allowlist-setting.png)
...
![Agent が提示した実装計画。制約と AGENTS.md のルールが反映されている](img/05-01-implementation-plan.png)

なお、スクリーンショットに写っている「Model query failed: RESOURCE_EXHAUSTED」の赤いバナーは混雑時によく出るものです。自動でリトライされるので、そのまま待てば問題ありません（詳しくは「うまくいかないときは」参照）。

計画に問題がなければ「はい」と返信します。Agent は Task List を作って作業を進めます。ウォームアップで **Always allow** を選んでいても、`build.gradle.kts` のような**実行可能ファイル（executable files）への書き込みは別の許可**として扱われ、あらためて確認が出ます。ここでも **Always allow** を選んで進めましょう（差分のレビューは完了後の Changes パネルでまとめて行います）。

<img src="img/05-02-plan-approved-flow.png" width="600" alt="「はい」と返信すると Task List を作成して作業が進む。build.gradle.kts への書き込みでは executable ファイル用の許可があらためて求められるので Always allow を選ぶ" />

この選択も Agent Permissions に反映されます。変わるのは **Write executable files in the project** だけで、ファイル書き込みの許可が「ソースファイル」「実行可能ファイル」など対象ごとに分かれて管理されていることがわかります。

![Always allow を選ぶと Agent Permissions の Write executable files in the project が Always allow に変わる](img/05-03-executable-permission-setting.png)
...
![作業完了後の画面。Walkthrough と Changes パネル（Keep All / Revert All）](img/05-04-changes-keep-revert.png)
...
<img src="img/05-05-photo-picker.png" width="400" alt="Photo Picker が開いた画面。選択した写真だけにアクセスできる旨のバナーが表示される" />

写真を選んで表示されれば成功です。

<img src="img/05-06-photo-picked.png" width="400" alt="選んだ写真がアプリに表示された画面" />

> aside negative
> エミュレータに写真がない場合は、**エミュレータのカメラアプリで何枚か撮影しておく**か、PC から画像ファイルをエミュレータ画面にドラッグ＆ドロップすると追加できます。

動いたら、ウォームアップと同様に「ここまでの変更をコミットして」と Agent に頼んでコミットしておきましょう。コミットメッセージは変更内容から生成されます（ウォームアップで allow list に追加済みなら、git コマンドは確認なしで実行されます）。

<img src="img/05-07-photo-picker-committed.png" width="460" alt="Agent が git status、git add、git commit を確認なしで実行し、Photo Picker 実装のコミットメッセージを生成している" />
...
> aside positive
> 制約を先に決めてあったからこそ、差分を「制約に違反していないか」という明確な観点でチェックできます。この**検証できる形で依頼して、検証する**というループは、このハンズオンで一番持ち帰ってほしい習慣です。

実行して、撮影 → 表示まで確認しましょう。

<img src="img/05-08-camera-capture.png" width="400" alt="「カメラで撮影」ボタンが追加され、撮影した写真が表示された画面" />

> aside negative
> エミュレータのカメラには擬似的な3D空間が映ります。実機ならより実感が湧きます。

動いたら、ここでも Agent に頼んでコミットしておきましょう。
...
<img src="img/06-01-emoji-row.png" width="400" alt="画面下部に絵文字パレットが横並びし、選択中の絵文字が枠線で強調されている" />
...
<img src="img/06-02-emoji-stamped.png" width="400" alt="写真の上に星とハートのスタンプが複数配置された画面" />
...
* **絵文字のスタンプを増やす**：パレットの絵文字を増やす。数が多くなったら選び方の UI をどうするかも Agent に相談してみましょう
...
* **Android Skills を試す**：同梱の Android スキルが得意なタスク（エッジツーエッジ対応、テスト整備など）を頼んで、スキルが使われる様子を観察する（入力欄の `@` で明示的に呼び出すこともできます）
...
* **小さく頼んで、観点を決めてレビュー**。全部を精読しなくても、チェックすべき点が決まっていれば差分は確認できる
