...
![Recent Chats に2つの会話が並んだ状態。「+」で会話を追加し、Recent Chats で切り替える](img/09-parallel-chats.png)
...
![表示モードを Project に切り替え、ルートを右クリックして New > File](img/19-create-agents-md.png)
...
![入力欄で @ を入力すると、スキルと Saved Prompts、ファイルの候補が表示される](img/16-skills-list.png)

一覧からスキルを選ぶと、チップとして入力欄に入ります。

![選択したスキルはチップとして入力欄に入る](img/22-skill-selected.png)

例えば `edge-to-edge` スキルを付けてエッジツーエッジ対応を依頼すると、Agent がまずスキルを読み込み（Activate Skill）、その手順に沿って作業を進めます。実行までは、後の自由演習で試してみてください。

![Agent が edge-to-edge スキルを読み込んで（Activate Skill / Read skill）作業を進める様子](img/23-skill-activated.png)
